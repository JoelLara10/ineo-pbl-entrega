...
export default function EnfermeriaScreen() {
  ...
  const loadPatients = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      console.error('Error loading nursing patients:', error);
  ...
  const totalPatients =
    patientsByArea.consulta.length +
    patientsByArea.urgencias.length +
    patientsByArea.hospitalizados.length;
  ...
  const handlePatientClick = (item) => {
    ...
    navigate(`/enfermeria/paciente/${item.id_atencion}/${item.Id_exp}`, {
      state: {
        id_atencion: item.id_atencion,
        Id_exp: item.Id_exp,
      },
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <button type="button" onClick={() => navigate(-1)} style={styles.iconButton}>
          <FiArrowLeft size={20} />
        </button>
        <div>
          ...
          return (
            <section key={section.key} style={styles.section}>
              <div style={{ ...styles.sectionHeader, borderLeftColor: section.color }}>
                ...
                <span style={{ ...styles.countBadge, backgroundColor: section.color }}>
                  {sectionData.length}
                </span>
              </div>
              ...
              {totalPages > 1 ? (
                <div style={styles.pagination}>
                  <button
                    type="button"
                    onClick={() => updatePage(section.key, page - 1)}
                    disabled={page === 1}
