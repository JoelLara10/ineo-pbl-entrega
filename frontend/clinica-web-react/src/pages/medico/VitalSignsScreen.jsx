...
import { FiActivity, FiArrowLeft, FiHeart, FiRefreshCw, FiSave, FiShield, FiUser } from 'react-icons/fi';
...
const CACHE_PREFIX = 'ineo_web_cache_medico_vital_signs_';
...
function getCachedValue(key) {
  ...
  try {
    ...
    console.error('Error reading medico vital signs cache:', error);
    return null;
  }
}
...
function setCachedValue(key, data, ttl = CACHE_TTL) {
  ...
  try {
    ...
    console.error('Error saving medico vital signs cache:', error);
  }
}
...
export default function VitalSignsScreen() {
  ...
  const [formData, setFormData] = useState({ ta: '', fc: '', fr: '', temp: '', spo2: '', peso: '', talla: '' });
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        console.error('Error loading medico vital signs history:', error);
      } finally {
        setLoadingHistory(false);
      }
    };
  ...
  const reloadHistory = async () => {
    ...
    try {
      ...
      console.error('Error reloading medico vital signs history:', error);
    } finally {
      setLoadingHistory(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const historyResponse = await api.get(`/appointments/${idAtencion}/vital-signs`);
        const nextHistory = historyResponse.data || [];
        setCachedValue(String(idAtencion), nextHistory);
        setHistory(nextHistory);
      }
    } catch (error) {
      console.error('Error saving medico vital signs:', error);
  ...
  const fieldConfig = [
    { key: 'ta', label: t('vitalSigns.ta'), placeholder: '120/80' },
    { key: 'fc', label: t('vitalSigns.fc'), placeholder: 'lpm' },
    { key: 'fr', label: t('vitalSigns.fr'), placeholder: 'rpm' },
    { key: 'temp', label: t('vitalSigns.temp'), placeholder: '36.5' },
    { key: 'spo2', label: t('vitalSigns.spo2'), placeholder: '98' },
    { key: 'peso', label: t('vitalSigns.peso'), placeholder: '70' },
    { key: 'talla', label: t('vitalSigns.talla'), placeholder: '1.70' },
  ];
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        ...
        <div><div style={styles.headerEyebrow}>MÉDICO</div><h1 style={styles.headerTitle}>{t('vitalSigns.title')}</h1></div>
      ...
      <section style={styles.patientCard}>
        ...
        <div><h2 style={styles.patientName}>{t('vitalSigns.selectedPatient')}</h2><p style={styles.patientMeta}>{patientLabel}</p></div>
      </section>

      {errorMessage ? <div style={styles.errorCard}>{errorMessage}</div> : null}

      <section style={styles.mainCard}>
        <div style={styles.cardHeader}><FiHeart size={20} /><strong>{t('vitalSigns.newRecord')}</strong></div>
        ...
        <div style={styles.cardFooter}>
          <button type="button" style={styles.cancelButton} onClick={() => navigate(-1)}>{t('vitalSigns.back')}</button>
          <button type="button" style={styles.saveButton} onClick={handleSubmit} disabled={!idAtencion || loading}><FiSave size={18} /><span>{loading ? t('vitalSigns.saving') : t('vitalSigns.save')}</span></button>
        </div>
      </section>
      ...
      <section style={styles.historyCard}>
        ...
        <div style={styles.historyBody}>
          {loadingHistory ? <div style={styles.statusBox}>{t('vitalSigns.loadingHistory')}</div> : history.length === 0 ? <div style={styles.statusBox}>{t('vitalSigns.noRecords')}</div> : history.map((item, index) => (
...
const styles = {
  ...
  patientCard: { marginTop: '20px', padding: '18px 20px', backgroundColor: '#fff', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '16px', boxShadow: '0 12px 32px rgba(15, 23, 42, 0.08)', borderLeft: '5px solid #ef4444' },
  patientAvatar: { width: '56px', height: '56px', borderRadius: '16px', backgroundColor: '#ef4444', display: 'grid', placeItems: 'center', flexShrink: 0 },
  ...
  cardHeader: { display: 'flex', alignItems: 'center', gap: '10px', padding: '18px 22px', color: '#fff', background: 'linear-gradient(135deg, #f56565 0%, #ed8936 100%)' },
  formGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px', padding: '22px' },
  fieldGroup: { display: 'flex', flexDirection: 'column', gap: '8px' },
  fieldLabel: { fontWeight: 700, color: '#1e293b' },
  fieldInput: { padding: '12px 14px', borderRadius: '14px', border: '1px solid #cbd5e1', backgroundColor: '#fff', font: 'inherit', color: '#0f172a' },
  cardFooter: { padding: '0 22px 22px', display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' },
  ...
  historyHeader: { display: 'flex', alignItems: 'center', gap: '10px', padding: '18px 22px', color: '#fff', background: 'linear-gradient(135deg, #4299e1 0%, #3182ce 100%)' },
  historyCount: { marginLeft: 'auto', padding: '4px 10px', borderRadius: '999px', backgroundColor: 'rgba(255,255,255,0.2)', fontSize: '12px' },
  historyBody: { padding: '18px' },
  statusBox: { padding: '28px', borderRadius: '18px', textAlign: 'center', backgroundColor: '#f8fafc', color: '#64748b' },
  historyItem: { padding: '16px', borderRadius: '18px', backgroundColor: '#f8fafc', marginBottom: '14px' },
  historyDate: { color: '#475569', fontSize: '13px', marginBottom: '12px' },
  historyGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px' },
  metricCard: { padding: '12px', borderRadius: '14px', backgroundColor: '#fff', border: '1px solid #e2e8f0' },
