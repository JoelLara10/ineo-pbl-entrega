...
function getCachedValue(key) {
  ...
  try {
    ...
    console.error('Error reading medico patient cache:', error);
    return null;
  }
}
...
function setCachedValue(key, data, ttl = CACHE_TTL) {
  ...
  try {
    ...
    console.error('Error saving medico patient cache:', error);
  }
}
...
export default function PatientDetailScreen() {
  ...
  useEffect(() => {
    if (!routeIdExp || !routeIdAtencion) return;
  ...
  const patientName = useMemo(() => (
    [patientData.papell, patientData.sapell, patientData.nom_pac].filter(Boolean).join(' ') || 'Paciente'
  ), [patientData]);
              ...
              {modules.map((module) => {
                ...
                return (
                  <button
                    key={module.title}
                    type="button"
                    onClick={() => navigate(module.path, { state: { id_atencion: routeIdAtencion, Id_exp: routeIdExp } })}
                    style={{ ...styles.moduleCard, backgroundColor: module.background }}
                  >
                    <div style={{ ...styles.moduleIcon, backgroundColor: `${module.color}18`, color: module.color }}><ModuleIcon size={24} /></div>
...
const styles = {
  ...
  errorCard: { marginTop: '20px', padding: '20px', backgroundColor: '#fff7ed', border: '1px solid #fdba74', color: '#9a3412', borderRadius: '18px' },
  mainCard: { marginTop: '20px', padding: '24px', backgroundColor: '#fff', borderRadius: '22px', boxShadow: '0 14px 36px rgba(15, 23, 42, 0.08)' },
  patientHeader: { display: 'flex', alignItems: 'center', gap: '18px', marginBottom: '22px' },
  avatar: { width: '74px', height: '74px', borderRadius: '999px', background: 'linear-gradient(135deg, #2563eb 0%, #7c3aed 100%)', display: 'grid', placeItems: 'center', flexShrink: 0 },
  patientInfo: { minWidth: 0 },
  patientName: { margin: 0, color: '#0f172a', fontSize: '24px', fontWeight: 800 },
  expBadge: { display: 'inline-block', marginTop: '8px', padding: '6px 12px', borderRadius: '999px', backgroundColor: '#dbeafe', color: '#1d4ed8', fontWeight: 700, fontSize: '13px' },
  infoGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px', marginBottom: '18px' },
  infoItem: { padding: '16px', borderRadius: '16px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' },
  infoLabel: { display: 'block', color: '#64748b', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' },
  infoValue: { color: '#0f172a', fontSize: '15px' },
  warningBox: { padding: '16px', borderRadius: '16px', backgroundColor: '#fef2f2', marginBottom: '14px' },
  infoBoxBlue: { padding: '16px', borderRadius: '16px', backgroundColor: '#eff6ff', marginBottom: '14px' },
  infoBoxGreen: { padding: '16px', borderRadius: '16px', backgroundColor: '#f0fdf4' },
  boxTitleRow: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' },
  boxText: { margin: 0, color: '#334155', lineHeight: 1.5 },
  boxMeta: { margin: '8px 0 0', display: 'flex', alignItems: 'center', gap: '6px', color: '#475569' },
  listWrap: { display: 'flex', flexDirection: 'column', gap: '4px' },
  listItem: { color: '#334155' },
  modulesSection: { marginTop: '22px' },
  modulesHeading: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' },
  modulesTitle: { margin: 0, fontSize: '20px', color: '#1e293b' },
  modulesGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' },
  moduleCard: { border: 'none', borderRadius: '18px', padding: '18px', textAlign: 'left', boxShadow: '0 10px 28px rgba(15, 23, 42, 0.06)' },
  moduleIcon: { width: '52px', height: '52px', borderRadius: '16px', display: 'grid', placeItems: 'center', marginBottom: '12px' },
  moduleTitle: { display: 'block', marginBottom: '6px', fontSize: '16px' },
  moduleSubtitle: { color: '#475569' },
